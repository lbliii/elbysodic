...
| Continuity proposals, citations, affected objects, reviews, and canon | Proposal author/face, source community/thread/post, affected typed object, reviewer/face, approver, or approved-public proposal disagree. | Continuity workflow and export services. | v27 tenant-pair triggers plus content-free tenant integrity diagnostics. | Repository rejection covers every allowed link type; raw storage, legacy drift audit, lifecycle/privacy, export, and migration parity are covered in `tests/test_continuity_backend.py` and `tests/test_tenant_repository.py`. | Invalid legacy rows block deliberate future upgrades; no content is auto-moved or printed. |

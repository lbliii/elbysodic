...
| Continuity Graph / canon review routes | Scene outcome proposals, source citations, affected-object links, review notes, staff actor identity, notification targets, and public canon visibility. | Not implemented. Future member views must show only proposals and source links the member can already access. | Not implemented. Future owner/participant controls must not grant staff review authority. | Not implemented. Future staff review must use named capabilities and hide inaccessible source titles/notes from unauthorized viewers. | No public route family exists yet; future routes must prove public, inactive, and cross-community attempts cannot see private or unreviewed canon. | readiness contract only; implementation gated by `docs/architecture/continuity-graph-readiness.md` |

...
| `/notifications` | Membership-specific inbox and unread counts, wanted-interest notes, plotting-room targets, watched-thread and mention snippets. | Own visible notifications only; registered notification kinds must satisfy their service-owned target contract before rendering, counting, redirecting, or marking read. Unregistered kinds, malformed rows, and forged inaccessible wanted/room targets are hidden, and inbox limits apply after target visibility filtering. | Same as member. | Staff still sees own inbox, not global inbox. | No other membership notifications; inaccessible, unregistered, or malformed targets do not contribute unread counts or crowd visible rows out of the page. | covered for own inbox, unread/read state, watched-thread replies, mentions, plotting-room leakage, wanted-interest leakage, hidden-row inbox-window parity, malformed target rows, and unregistered notification kinds |

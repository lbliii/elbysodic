...
def recovery_view(
    ...
    *,
    ...
    if communities:
        target_name = target.name if target is not None else communities[0].name
        title = f"That {labels.object_label} lives in {target_name}."
        summary = (
            f"{slug} is not part of {viewer.community.name}, but it exists in another "
            "program on this studio network."
        )
        detail = (
            "Switch realms to open it there, or return to this program's local lane."
            if switch_action is not None
            else "Return to this program's local lane to keep browsing here."
        )
    else:
